...
class Tolokers2(InMemoryDataset):
    ...
    def process(self):
        ...
        for i in range(df_y.shape[0]):
            ...
            new_value = float(df_y["was_banned"][i])
        ...
        qt_numerical = qt.fit_transform(numerical_features.copy())
        imputer = SimpleImputer(
            missing_values=np.nan, strategy="most_frequent", copy=False
        )
        qt_numerical_nonnan = imputer.fit_transform(qt_numerical.copy())

        features = np.concatenate(
            [qt_numerical_nonnan, fraction_features, categorical_features], axis=1
        )
        feature_map = {
