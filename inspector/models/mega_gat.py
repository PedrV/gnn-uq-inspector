...
@register_model("megagat", task_type="node")
class MegaGATModel(torch.nn.Module):
    def __init__(
        self,
        in_dim,
        hidden_dim,
        ...
        **kwargs,
        ...
        for i in range(num_layers):
            # We set out_channels to head_dim because concat=True will
            # result in head_dim * heads = hidden_dim
            self.layers.append(
                GATv2Conv(
                    hidden_dim,
                    head_dim,
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        for i in range(self.num_layers):
            # Residual/Wrapper Logic: Norm -> GAT -> FF -> Add
            h = self.norms[i](x)
            h = checkpoint(self.layers[i], h, edge_index, use_reentrant=False)
